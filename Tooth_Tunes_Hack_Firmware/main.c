...
// Original code retrieved from http://elm-chan.org/works/sd8p/report.html
// Modified by Joe Grand 1/24/17 for Tooth Tunes Hack

// Compiled with avr-gcc 4.9.1
// * Added #define to fix depreciated prog_char
// * Change main() to play the first detected file only one time (no repeat)
// * Change PB5 from external /RESET pin to GPIO (prevents in-circuit programming)
// * Assert PB5 at the end of the song, which will cause the MAX16054 to power off our circuit
// * Commented out unneeded frequency/OSCCAL adjustment code

// Programmed with USBtinyISP 2.0:
// * avrdude -c usbtiny -p t85 -U flash:w:sd8p_mo.hex:i
// * avrdude -c usbtiny -p t85 -V -U lfuse:w:0xe1:m -U hfuse:w:0x7d:m


#define __PROG_TYPES_COMPAT__  /* fixes depreciated prog_char */
...
//FUSES = {0xE1, 0x7D, 0xFF};	/* Fuse bytes for mono: Low, High and Extended (disable external reset) */
#else			/* Dual output */
...
int main (void)
{
	...
	PORTB = 0b001001;		/* Initialize port: - - L L H L L P */
	DDRB  = 0b111110;

	sei();

    /* Just play the first detected file one time (no repeat) */
    for (;;) {
            ...
            while (res == FR_OK) {				/* Repeat in the dir */
                ...
                while (res == FR_OK) {				/* Play all wav files in the dir */
                    ...
                    if (!(Fno.fattrib & (AM_DIR|AM_HID)) && strstr(Fno.fname, ".WAV")) {
                        ...
                        PORTB |= (1 << 5);      /* Set PB5 to HIGH, which will cause the MAX16054 to power off our circuit */
                        while(1) wdt_reset();   /* We should never get here */
                    }
                }
            }
        }
